Shader "DryCycle/FogComposite"
{
    ...
    Category
    {
        ...
        SubShader
        {
            // One explicit grab is intentional: this shader is a final world composite,
            // not a translucent white sprite. It receives the complete world rendered
            // below GrabShaders, computes transmittance, then writes scene+fog back.
            GrabPass { "_DryCycleFogScene" }
            ...
            Pass
            {
                ...
                {
                    float2 span = max(abs(_spriteRect.zw - _spriteRect.xy), 0.0001);
                ...
                float2 DomainWarpPx(float2 worldPx, float time)
                {
                    ...
                    float wx = tex2D(_NoiseTex, p * float2(0.83, 1.17) + float2(time * 0.0041, time * -0.0017)).r;
                    float wy = tex2D(_NoiseTex, p.yx * float2(1.31, 0.71) + float2(time * -0.0027, time * 0.0033) + 0.37).r;
                    float2 warp = float2(wx, wy) * 2.0 - 1.0;

                    float wx2 = tex2D(_NoiseTex2, p * 0.43 + float2(time * -0.0011, 0.19)).g;
                    float wy2 = tex2D(_NoiseTex2, p.yx * 0.47 + float2(0.61, time * 0.0015)).b;
                ...
                float VisualFogDensity(float2 roomUV, float2 worldPx, float pseudoDepth, float jitter)
                {
                    const int Steps = 12;
                    ...
                    for (int s = 0; s < Steps; s++)
                    {
                        ...
                        float depthParallax = (z - 0.5) * lerp(34.0, 82.0, pseudoDepth);
                        float2 sampleWorld = worldPx + warpPx * lerp(0.35, 1.0, z);
                        sampleWorld += float2(depthParallax * 0.41, depthParallax * -0.17);

                        float2 sampleRoomUv = sampleWorld / max(_DryCycleRoomSizePx, 1.0.xx);
                        float fluid = tex2D(_DryCycleFogDensityTex, saturate(sampleRoomUv)).r;
                        ...
                        float3 macroCoord = float3(
                            sampleWorld / 430.0 + float2(time * 0.0032, time * -0.0011),
                            z * 0.91 + time * 0.0017);
                        float4 macro = SampleVolume(macroCoord);

                        float3 detailCoord = float3(
                            sampleWorld / 137.0 + float2(time * -0.0071, time * 0.0029),
                            z * 2.73 - time * 0.0037);
                        float4 detail = SampleVolume(detailCoord + macro.gbr * 0.19);

                        float body = saturate(macro.r * 1.30 - (1.0 - detail.b) * 0.38 + 0.05);
                        body = pow(body, lerp(1.45, 0.82, macro.g));
                        accumulated += body * lerp(0.56, 1.18, fluid);
                    }

                    float density = accumulated / (float)Steps;
                    float wallDistance = tex2D(_DryCycleFogObstacleTex, saturate(roomUV)).g;
                    density += pow(saturate(1.0 - wallDistance), 2.0) * 0.075;
                    return saturate(density * 1.42);
                }

                float ObstacleAtWorld(float2 worldPx)
                {
                    float2 uv = worldPx / max(_DryCycleRoomSizePx, 1.0.xx);
                ...
                float SegmentVisibility(float2 fromPx, float2 toPx)
                {
                    ...
                    for (int i = 1; i <= OcclusionSteps; i++)
                    {
                        ...
                        blocked = max(blocked, step(0.5, ObstacleAtWorld(p)));
                    }
                    return 1.0 - blocked;
                }
                    ...
                    out float3 coloredScattering)
                {
                    ...
                    for (int i = 0; i < 8; i++)
                    {
                        ...
                        float radial = Smooth01(1.0 - distancePx / radius);
                        radial = pow(radial, 0.66);
                        float visible = SegmentVisibility(light.xy, worldPx);
                        float localReveal = saturate(radial * light.w * visible);

                        reveal = 1.0 - (1.0 - reveal) * (1.0 - localReveal);
                        coloredScattering += _DryCycleFogLightColors[i].rgb *
                            localReveal * lerp(0.10, 0.42, visualDensity);
                    }
                }
                ...
                float EvaluateAwareness(float2 worldPx)
                {
                    ...
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        float distancePx = distance(worldPx, awareness.xy);
                        if (distancePx >= awareness.z)
                            continue;

                        float radial = Smooth01(1.0 - distancePx / max(1.0, awareness.z));
                        float visible = SegmentVisibility(awareness.xy, worldPx);
                        float localReveal = radial * awareness.w * visible;
                        reveal = 1.0 - (1.0 - reveal) * (1.0 - localReveal);
                    }
                ...
                half4 frag(v2f i) : SV_Target
                {
                    ...
                    float2 worldPx = roomUV * _DryCycleRoomSizePx;
                    float pseudoDepth = DecodePseudoDepth(LevelUV(screenUV));

                    float4 scene = tex2Dproj(_DryCycleFogScene, UNITY_PROJ_COORD(i.grabPos));
                    ...
                    float jitter = tex2D(_NoiseTex2, frac(screenUV * (_screenSize / 256.0))).r;
                    float visualDensity = VisualFogDensity(roomUV, worldPx, pseudoDepth, jitter);
                    ...
                    float depthFactor = lerp(0.86, 1.28, pseudoDepth);
                    float extinction = fog * 1.05 + dense * 6.22;
                    float baseTransmittance = exp(-extinction * depthFactor);

                    // At full DenseFog force the high-contrast floor into the same
                    // ~0.2% regime as the earlier CPU extinction prototype.
                    float denseCeiling = lerp(1.0, 0.0022, dense);
                    baseTransmittance = min(baseTransmittance, denseCeiling);

                    float lightReveal;
                    float3 lightScattering;
                    EvaluateFogLights(worldPx, visualDensity, lightReveal, lightScattering);
                    float awarenessReveal = EvaluateAwareness(worldPx);
                    float reveal = 1.0 - (1.0 - lightReveal) * (1.0 - awarenessReveal);

                    float transmittance = lerp(baseTransmittance, 0.985, reveal);

                    // Density changes the appearance and internal lighting of the fog,
                    // not the gameplay transmittance. Thick lobes are slightly brighter;
                    // valleys are darker and cooler instead of becoming transparent.
                    float densityShade = smoothstep(0.16, 0.88, visualDensity);
                    float shade = lerp(0.77, 1.12, densityShade);
                    float3 fogColor = _DryCycleFogColor.rgb * shade;
                    fogColor = lerp(fogColor, _DryCycleFogColor.rgb * 0.90, (1.0 - visualDensity) * 0.25);

                    float3 scattering = fogColor + lightScattering * (0.48 + dense * 0.22);
                    float3 finalColor = scene.rgb * transmittance + scattering * (1.0 - transmittance);
