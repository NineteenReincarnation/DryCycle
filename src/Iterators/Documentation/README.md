...
目前完成任务书第 81–83 节的**定义注册、Runtime 与游戏绑定、Body / Arm / Pose**。定义注册后，指定房间完成加载时会创建独立 Runtime、绑定 Oracle 和默认身体，并在房间卸载、Session 结束、注销或插件停用时销毁。

默认身体可移动、碰撞、接受姿势和机械臂约束，尚无 Graphics 绘制。代码和托管检查已通过，真实 Unity 游戏内验收待做。
...
当前 API 为前三阶段开发接口。后续优先以新增接口演进；正式发布后，已有 public API 的改名或移除必须提供弃用与迁移期。`FrameworkVersion`、`ApiVersion` 和能力查询尚未实现。
