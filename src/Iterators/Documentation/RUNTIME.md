...
第二阶段提供实例创建、Context、Oracle / Room 绑定与销毁；第三阶段接入 [Body / Arm / Pose](BODY.md)。所有操作和回调在 Unity 主线程同步执行；不提供后台写入或异步生命周期保证。
...
| `State` | 框架行为与回调 |
| --- | --- |
| `RuntimeCreated` | 绑定 Runtime、Context、Oracle，将宿主加入 Room，创建并初始化 Body / Arm，然后调用 `OnCreate`。 |
| `Initializing` | 调用 `OnInitialize`。 |
| `Initialized` | 调用 `OnRoomReady`。 |
| `Active` | 调用一次 `OnActivate`；之后每次宿主 Update 调用 OnUpdate、Body / Arm 控制与物理、OnLateUpdate。 |
...
| `Body / Arm` | 当前身体与机械臂约束，在正常 OnCreate 前已初始化；销毁后保留已释放组件供状态查询。 |
...
| `Body / Arm` | 与 Runtime 的组件属性相同；部分创建失败时可能为空，销毁后组件 IsDestroyed 为 true。 |
...
清理顺序：设置 Destroying 与原因 → Runtime.OnDestroy → Arm.OnDestroy → Body.OnDestroy → 销毁并移除宿主 → 移除 Room / Oracle 索引和活动列表 → 清空 Context 的游戏引用 → Destroyed。

OnDestroy 抛异常仍会继续清理。初始化与更新回调抛异常只结束当前实例，错误带有 ID 和阶段日志。工厂返回 null、外来 Context 或复用实例会被拒绝；框架不会销毁工厂错误返回的其他实例。插件内部清理先于原版房间卸载和 Session 关闭，原版流程仍继续执行。
...
第三阶段的 Body / Arm / Pose 已接入，宿主使用游戏物理但尚无绘制，不运行原版 Oracle AI，也不持久化进存档。Graphics 属于第四阶段，行为、对话、环境和存档属于后续阶段。当前托管检查调用实际游戏程序集及 Hook，但不能替代 Unity 内的房间加载、原版迭代器共存和其他 Mod 兼容验收。
