...
{
    private static readonly ConditionalWeakTable<Player, ThirstMeter> Meters = new();
        ...
        : base(hud)
    {
        ...
        {
            _outer[i] = new global::HUD.HUDCircle(hud, global::HUD.HUDCircle.SnapToGraphic.FoodCircleA, hud.fContainers[1], 0)
            {
                forceColor = WaterColor
            };
            _inner[i] = new global::HUD.HUDCircle(hud, global::HUD.HUDCircle.SnapToGraphic.FoodCircleB, hud.fContainers[1], 0)
            {
                forceColor = WaterColor
            };
        }
    ...
    {
        if (Meters.TryGetValue(player, out _))
        {
            return;
        }

        ThirstMeter meter = new(hud, player);
        Meters.Add(player, meter);
        hud.AddPart(meter);
    }

    public static void TryReject(Player player)
    {
        if (player != null && Meters.TryGetValue(player, out ThirstMeter meter))
        {
            meter._rejectCounter = 55;
            meter._visibleCounter = 200;
        }
    }
    ...
    public override void Update()
    {
        ...
        float target = (inShelter || _visibleCounter > 0 || state.IsDrinking) ? 1f : foodFade;
        _fade = Mathf.Lerp(_fade, Mathf.Max(foodFade, target), 0.2f);
        ...
        for (int i = 0; i < _outer.Length; i++)
        {
            // Mirror FoodMeter spacing: after the second pip, leave a half-pip gap
            // to show the 2-pip hibernation requirement.
