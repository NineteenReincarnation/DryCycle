...
internal static class DB_NeutralBehaviorRuntime
{
    ...
    {
        if (p == null) return 0f;
        return Mathf.Clamp(0.17f + p.Conformity * 0.30f + (1f - p.Temperament) * 0.07f + p.Nerve * 0.04f,
            0.16f, 0.58f);
    }
    ...
    internal static float LooseFlockPreference(DB_Personality p, int nearby)
    {
        ...
        return Mathf.Clamp01(0.10f + p.Conformity * 0.52f + (1f - p.Temperament) * 0.10f +
    ...
    internal static bool ShouldOwn(in DB_FrameContext frame)
    {
        ...
        {
            if (bat != null && states.TryGetValue(bat, out State blocked))
                ToRoam(bat, blocked, clock, 55, 130);
            return false;
        }
        ...
        {
            if (clock < state.Until) return true;
            ToRoam(bat, state, clock, 55, 150);
        }
        if (clock < state.NextDecision) return false;
        ...
        if (Stable01(seed, serial * 193 + 0x6D31) > participate) return false;
        ...
        if ((pick -= peer) < 0f)
        {
            Begin(bat, state, DB_NeutralMode.PeerDrift, clock, StableInt(seed, serial * 43 + 0x5311, 32, 73));
            state.PeerCooldown = state.Until + StableInt(seed, serial * 59 + 0x6323, 120, 321);
        }
        else if ((pick -= flock) < 0f)
        {
            Begin(bat, state, DB_NeutralMode.LooseFlock, clock, StableInt(seed, serial * 67 + 0x2741, 45, 111));
            state.FlockCooldown = state.Until + StableInt(seed, serial * 71 + 0x7331, 180, 421);
        }
        else
        {
            Begin(bat, state, DB_NeutralMode.ShortSwarm, clock, StableInt(seed, serial * 83 + 0x3559, 28, 76));
    ...
    internal static bool ApplyOwnedBehavior(DB_Creature bat)
    {
        ...
        switch (state.Mode)
        {
            case DB_NeutralMode.PeerDrift:
                if (!PeerGoal(bat, state, candidates, out goal)) { ToRoam(bat, state, clock, 70, 170); return false; }
                break;
            case DB_NeutralMode.LooseFlock:
                if (!FlockGoal(bat, state, candidates, out goal)) { ToRoam(bat, state, clock, 80, 190); return false; }
                break;
        ...
        if (Guide(bat, goal, speed, state.Side)) return true;
        ToRoam(bat, state, clock, 80, 190);
        return false;
    }

    private static State For(DB_Creature bat, int clock)
    {
        State s = states.GetOrCreateValue(bat);
        if (ReferenceEquals(s.Room, bat.room)) return s;
        s.Room = bat.room;
        s.Mode = DB_NeutralMode.Roam;
        s.Started = clock;
        s.Until = 0;
        s.NextDecision = clock + StableInt(Seed(bat), 0x4117, 70, 181);
        s.Serial = 0;
        s.PeerCooldown = s.FlockCooldown = s.SwarmCooldown = 0;
        s.Peer = null;
        s.Anchor = bat.mainBodyChunk.pos;
        s.Side = Stable01(Seed(bat), 0x51A9) < 0.5f ? -1 : 1;
        return s;
    }

    private static void Begin(DB_Creature bat, State s, DB_NeutralMode mode, int clock, int duration)
    {
        s.Mode = mode;
        s.Started = clock;
        s.Until = clock + Math.Max(1, duration);
        s.Side = Stable01(Seed(bat), s.Serial * 101 + 0x1823) < 0.5f ? -1 : 1;
        s.Peer = null;
        s.Anchor = bat.mainBodyChunk.pos;
    }

    private static void ToRoam(DB_Creature bat, State s, int clock, int minDelay, int maxDelay)
    {
        s.Mode = DB_NeutralMode.Roam;
        s.Started = clock;
        s.Until = 0;
        s.Peer = null;
        s.NextDecision = Math.Max(s.NextDecision,
            clock + StableInt(Seed(bat), s.Serial * 109 + 0x1937, minDelay, maxDelay + 1));
    }

    private static bool PeerGoal(DB_Creature bat, State s, IReadOnlyList<DB_Creature> peers, out Vector2 goal)
    {
        goal = default;
        if (!ValidPeer(bat, s.Peer)) s.Peer = SelectPeer(bat, peers, s.Serial);
        DB_Creature peer = s.Peer;
        ...
        if (distance < 50f)
        {
            ...
            Vector2 tangent = new Vector2(-away.y, away.x) * s.Side;
        ...
        Vector2 back = peer.mainBodyChunk.vel.sqrMagnitude > 1f ? -peer.mainBodyChunk.vel.normalized * 18f : Vector2.zero;
        float vertical = StableRange(Seed(bat), s.Serial * 131 + 0x2517, -16f, 16f);
        goal = peer.mainBodyChunk.pos + back + new Vector2(s.Side * Mathf.Lerp(54f, 72f, 1f - bond), vertical);
        return true;
    }
    ...
    private static DB_Creature SelectPeer(DB_Creature bat, IReadOnlyList<DB_Creature> peers, int serial)
    {
        ...
        for (int i = 0; i < samples; i++)
        {
            ...
            float d = Vector2.Distance(bat.mainBodyChunk.pos, peer.mainBodyChunk.pos);
            if (d < 24f || d > PeerRange || (d > 95f && !bat.room.VisualContact(bat.mainBodyChunk.pos, peer.mainBodyChunk.pos))) continue;
            float bond = Mathf.Max(DB_SocialBond.GetBondStrength(bat, peer), DB_SocialBond.GetBondStrength(peer, bat));
            float score = (1f - Mathf.Clamp01(Mathf.Abs(d - 105f) / 140f)) * 0.65f + bond * 0.35f;
            if (score > bestScore) { bestScore = score; best = peer; }
        }
        return best;
    }

    private static bool FlockGoal(DB_Creature bat, State s, IReadOnlyList<DB_Creature> peers, out Vector2 goal)
    {
        goal = default;
        Vector2 center = Vector2.zero, velocity = Vector2.zero, separation = Vector2.zero;
        int contributors = 0;
        int samples = Math.Min(SampleLimit, peers.Count);
        int start = StableInt(Seed(bat), s.Serial * 157 + 0x5129, 0, peers.Count);
        for (int i = 0; i < samples; i++)
        {
            DB_Creature peer = peers[(start + i) % peers.Count];
            if (!ValidPeer(bat, peer)) continue;
            ...
            if (d <= 0.01f || d > FlockRange || (d > 90f && !bat.room.VisualContact(bat.mainBodyChunk.pos, peer.mainBodyChunk.pos))) continue;
            center += peer.mainBodyChunk.pos;
            velocity += peer.mainBodyChunk.vel;
            contributors++;
            if (d < SeparationRadius)
            {
                float strength = Mathf.Pow(1f - d / SeparationRadius, 2f);
                Vector2 away = -delta / d;
        ...
        Vector2 align = velocity.sqrMagnitude > 0.5f ? velocity.normalized * 38f : Vector2.right * s.Side * 32f;
        align.y *= 0.48f;
        if (Mathf.Abs(align.x) < 16f) align.x = s.Side * 16f;
        Vector2 cohesion = center - bat.mainBodyChunk.pos;
        Vector2 offset = align + new Vector2(Mathf.Clamp(cohesion.x * 0.12f, -24f, 24f), Mathf.Clamp(cohesion.y * 0.10f, -14f, 14f));
        offset += new Vector2(Mathf.Clamp(separation.x * 92f, -105f, 105f), Mathf.Clamp(separation.y * 82f, -92f, 92f));
        offset.x += s.Side * 12f;
        goal = bat.mainBodyChunk.pos + offset;
        return true;
    }

    private static Vector2 SwarmGoal(DB_Creature bat, State s, IReadOnlyList<DB_Creature> peers, int clock)
    {
        int elapsed = Math.Max(0, clock - s.Started);
        float phase = elapsed * 0.105f + StableRange(Seed(bat), s.Serial * 163 + 0x6197, 0f, 6.2831855f);
        Vector2 goal = s.Anchor + new Vector2(Mathf.Sin(phase) * 46f + s.Side * (24f + elapsed * 0.55f), Mathf.Sin(phase * 0.71f + 1.2f) * 18f);
        Vector2 separation = Separation(bat, peers);
        goal += new Vector2(separation.x * 78f, separation.y * 68f);
        return Custom.RestrictInRect(goal, new FloatRect(18f, 18f, bat.room.PixelWidth - 18f, bat.room.PixelHeight - 18f));
    }
    ...
    private static Vector2 Separation(DB_Creature bat, IReadOnlyList<DB_Creature> peers)
    {
        ...
        for (int i = 0; i < samples; i++)
        {
            ...
            float d = delta.magnitude;
            if (d <= 0.01f || d >= SeparationRadius) continue;
            Vector2 away = delta / d;
    ...
    private static bool Guide(DB_Creature bat, Vector2 goal, float speed, int side)
    {
        ...
        if (Obstructed(bat.room, bat.mainBodyChunk.pos + direction * 26f))
        {
            ...
            if (!Obstructed(bat.room, bat.mainBodyChunk.pos + preferred * 32f)) goal = bat.mainBodyChunk.pos + preferred * 66f;
            else if (!Obstructed(bat.room, bat.mainBodyChunk.pos - preferred * 32f)) goal = bat.mainBodyChunk.pos - preferred * 66f;
            else if (!Obstructed(bat.room, bat.mainBodyChunk.pos + Vector2.down * 32f)) goal = bat.mainBodyChunk.pos + Vector2.down * 60f;
            else if (!Obstructed(bat.room, bat.mainBodyChunk.pos + Vector2.up * 32f)) goal = bat.mainBodyChunk.pos + Vector2.up * 60f;
            else return false;
        }
        bat.burrowOrHangSpot = null;
        if (bat.AI.behavior != FlyAI.Behavior.Idle) bat.AI.ChangeBehavior(FlyAI.Behavior.Idle);
    ...
    private static bool Obstructed(Room room, Vector2 point)
    {
        ...
        if (tile.x < 0 || tile.y < 0 || tile.x >= room.TileWidth || tile.y >= room.TileHeight) return true;
        return room.GetTile(tile).Solid || (room.terrain != null && room.terrain.ObstructsTile(tile));
    }
    ...
    private static bool ValidPeer(DB_Creature source, DB_Creature peer)
    {
        ...
        if (source.abstractCreature.rippleLayer != peer.abstractCreature.rippleLayer &&
            ...
            return false;
        return DB_SocialRuntime.TryGetDebugState(peer, out DB_SocialDebugState social) && social.Eligible && social.Mode == DB_SocialMode.None;
    }

    private static int Seed(DB_Creature bat) => bat?.Personality?.VisualSeed ?? bat?.abstractCreature?.ID.RandomSeed ?? 0;
    private static float StableRange(int seed, int salt, float min, float max) => Mathf.Lerp(min, max, Stable01(seed, salt));
