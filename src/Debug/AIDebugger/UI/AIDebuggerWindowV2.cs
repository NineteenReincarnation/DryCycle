...
internal sealed class AIDebuggerWindowV2
{
    ...
    private readonly Dictionary<DebugEntityKey, int> genericLastSampleFrame = new();
    ...
    private int lastAnomalyFrame = int.MinValue;
    ...
    private void DrawCompact(RainWorldGame game, double overheadMs)
    {
        ...
        DrawToolbar(game, overheadMs, compact: true);
        ImGui.Separator();
        ...
        {
            ImGui.SameLine();
            ImGui.TextColored(new Num.Vector4(0.96f, 0.72f, 0.28f, 1f),
                AIDebugExtendedLocalization.T("status.frozen"));
        }
        if (AIDebugSimulationControl.Paused)
        {
            ImGui.SameLine();
            ImGui.TextColored(new Num.Vector4(0.96f, 0.46f, 0.28f, 1f),
                AIDebugExtendedLocalization.T("status.paused"));
        }
    ...
    private void DrawDocked(RainWorldGame game, double overheadMs)
    {
        ...
        ImGuiWindowFlags rootFlags = ImGuiWindowFlags.NoTitleBar | ImGuiWindowFlags.NoCollapse |
                                     ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove |
                                     ImGuiWindowFlags.NoBringToFrontOnFocus | ImGuiWindowFlags.NoNavFocus |
                                     ImGuiWindowFlags.NoSavedSettings;
        ImGui.Begin("DryCycle AI Observatory DockHost###AIDockHost", rootFlags);
        DrawToolbar(game, overheadMs, compact: false);
        ImGui.Separator();
    ...
    private void DrawToolbar(RainWorldGame game, double overheadMs, bool compact)
    {
        if (ImGui.Button(fullMode ? AIDebugLocalization.T("app.compact") : AIDebugLocalization.T("app.full")))
            fullMode = !fullMode;
        ImGui.SameLine();
        if (ImGui.Button(interactMode
                ? AIDebugExtendedLocalization.T("toolbar.live")
                : AIDebugExtendedLocalization.T("toolbar.interact")))
            interactMode = !interactMode;
        ImGui.SameLine();
        if (ImGui.Button(AIDebugSimulationControl.Paused
                ? AIDebugExtendedLocalization.T("toolbar.resume")
                : AIDebugExtendedLocalization.T("toolbar.pause")))
            AIDebugSimulationControl.Toggle(game);
        ImGui.SameLine();
        if (ImGui.Button(AIDebugExtendedLocalization.T("toolbar.step")))
            AIDebugSimulationControl.Step(game);
        ImGui.SameLine();
        if (ImGui.Button(freezeView
                ? AIDebugExtendedLocalization.T("toolbar.unfreeze")
                : AIDebugExtendedLocalization.T("toolbar.freeze")))
            ToggleFreeze();
        ImGui.SameLine();
        ...
        ImGui.SameLine();
        ImGui.TextDisabled($"{overheadMs:0.00} ms · F7/F6 · Alt+LMB · Tab");
    }

    private void DrawEntityBrowser(RainWorldGame game)
    {
        BeginDockWindow("window.browser", "AIEntityBrowser");
        ImGui.TextDisabled(AIDebugExtendedLocalization.T("browser.help"));
        ...
        for (int i = 0; i < entities.Count; i++)
            if (entities[i].pos.room == currentRoom && MatchesEntityFilter(entities[i])) DrawEntityRow(entities[i], false);
        ImGui.Separator();
    ...
    {
        BeginDockWindow("window.decision", "AIDecision");
        AIDebugSnapshot active = ActiveSnapshot();
        ...
        {
            ImGui.Separator();
            ImGui.TextDisabled("Why / Why Not");
            int start = Mathf.Max(0, events.Count - 10);
            for (int i = start; i < events.Count; i++)
            {
                AIDebugTraceEvent e = events[i];
                ImGui.BulletText(AIDebugExtendedLocalization.EventName(e.Name) + ": " + e.Detail);
    ...
    {
        BeginDockWindow("window.inspector", "AIInspector");
        AIDebugSnapshot active = ActiveSnapshot();
        if (active == null) { NoSelection(); ImGui.End(); return; }
    ...
    {
        BeginDockWindow("window.timeline", "AITimeline");
        using (AIDebugProfiler.Begin(AIDebugProfileCategory.Timeline))
        {
            ...
            {
                AIDebugTrace.Clear(selectedKey); frames.Clear(); events.Clear(); timelineCursor = -1;
                ImGui.End(); return;
            }
            ImGui.SameLine();
            float duration = frames.Count > 1 ? frames[frames.Count - 1].Time - frames[0].Time : 0f;
            ImGui.TextDisabled($"{frames.Count} · {duration:0.0}s · {AIDebugExtendedLocalization.T("timeline.history")}");
            DrawTimelineStrip();
            if (frames.Count > 0)
            {
                ...
                if (ImGui.SliderInt("Sample##Timeline", ref timelineCursor, 0, frames.Count - 1)) freezeView = true;
                ...
                {
                    Row("Frame / Time", $"{f.Frame} / {f.Time:0.000}s");
    ...
    {
        BeginDockWindow("window.events", "AIEvents");
    ...
    {
        BeginDockWindow("window.utility", "AIUtility");
        ...
        {
            ImGui.TableSetupColumn("Module"); ImGui.TableSetupColumn("Raw"); ImGui.TableSetupColumn("Smoothed");
            ImGui.TableSetupColumn("Weight"); ImGui.TableSetupColumn("Weighted"); ImGui.TableSetupColumn("Continuation"); ImGui.TableSetupColumn("Winner");
    ...
    {
        BeginDockWindow("window.perception", "AIPerception");
        ...
        {
            string[] heads = { "Creature", "Visible", "Since seen", "Find chance", "Priority", "Last seen", "Best guess", "Relationship" };
            for (int i = 0; i < heads.Length; i++) ImGui.TableSetupColumn(heads[i]);
            ImGui.TableHeadersRow();
            for (int i = 0; i < source.Count; i++)
            {
                AIDebugPerceptionRow p = source[i];
                ImGui.TableNextRow(); Cell(0, p.Name); Cell(1, p.VisualContact ? "YES" : "no");
    ...
    {
        BeginDockWindow("window.compare", "AICompare");
        ...
        {
            ImGui.TableSetupColumn("Field"); ImGui.TableSetupColumn("A"); ImGui.TableSetupColumn("B"); ImGui.TableHeadersRow();
    ...
    {
        BeginDockWindow("window.candidates", "AICandidates");
        candidates.Clear();
        ...
        if (ImGui.BeginTable("##CandidatesTable", 6, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.Resizable))
        {
            ...
            for (int i = 0; i < candidates.Count; i++)
            {
                ...
                ImGui.TableNextRow(); Cell(0, c.Set); Cell(1, c.Name); Cell(2, c.Valid ? "YES" : "NO");
                Cell(3, c.Score.ToString("0.000")); Cell(4, c.Winner ? "WINNER" : "—"); Cell(5, c.Reason);
            }
            ImGui.EndTable();
        }
        ImGui.End();
    ...
    {
        BeginDockWindow("window.captures", "AICaptures");
        ...
        ImGui.Separator();
        ImGui.TextDisabled("Conditional Breakpoints");
        ImGui.InputText(AIDebugExtendedLocalization.T("breakpoint.name") + "##BPName", ref breakpointName, 96);
        string preview = breakpointCategory < 0 ? "Any category" : ((AIDebugEventCategory)breakpointCategory).ToString();
        if (ImGui.BeginCombo("Category##BPCategory", preview))
        {
            if (ImGui.Selectable("Any category", breakpointCategory < 0)) breakpointCategory = -1;
            foreach (AIDebugEventCategory category in Enum.GetValues(typeof(AIDebugEventCategory)))
                if (ImGui.Selectable(category.ToString(), breakpointCategory == (int)category)) breakpointCategory = (int)category;
            ImGui.EndCombo();
        }
    ...
    {
        BeginDockWindow("window.settings", "AISettings");
        bool changed = false;
        ...
        ImGui.Separator();
        ImGui.Text($"{AIDebugExtendedLocalization.T("status.input_gate")}: {(AIDebugInputGate.Installed ? "OK" : "Unavailable")}");
        ImGui.Text($"{AIDebugExtendedLocalization.T("status.world_step")}: {(AIDebugSimulationControl.Paused ? "Paused" : "Ready")}");
        ImGui.End();
    }
    ...
    private void SampleOne(AbstractCreature creature)
    {
        ...
        if (genericLastSampleFrame.TryGetValue(key, out int last) && frame - last < 4) return;
        genericLastSampleFrame[key] = frame;
        ...
        string utility = ai?.utilityComparer?.highestUtilityTracker?.module?.GetType().Name ?? "—";
        ...
        float highest = ai?.utilityComparer?.highestUtilityTracker?.SmoothedUtility() ?? 0f;
        AIDebugTrace.Sample(creature, new AIDebugTraceFrame(creature.Room?.name, pos, vel, pos,
            controller, utility, "—", "—", controller, highest, 0f, 0f, 0f));
    }

    private void DetectAnomalies()
    {
        if (!AIDebugSettings.DetectAnomalies || selected == null || frames.Count == 0) return;
        int frame = frames[frames.Count - 1].Frame;
        if (frame == lastAnomalyFrame) return;
        lastAnomalyFrame = frame;
    ...
    private void RefreshEntities(RainWorldGame game)
    {
        ...
        if (!hasSelection && entities.Count > 0)
        {
            ...
            int room = game.cameras != null && game.cameras.Length > 0 && game.cameras[0]?.room != null
                ? game.cameras[0].room.abstractRoom.index : int.MinValue;
            for (int i = 0; i < entities.Count; i++)
            {
                if (entities[i].pos.room != room) continue;
    ...
    private void DrawTimelineStrip()
    {
        ...
        for (int i = 0; i < frames.Count; i++)
        {
            ...
            draw.AddRectFilled(new Num.Vector2(start.X + i * step, start.Y + 6f),
                new Num.Vector2(start.X + (i + 1) * step + 0.5f, start.Y + height - 6f), color);
        }
    ...
    {
        for (int i = 0; i < entities.Count; i++) if (DebugEntityKey.From(entities[i]) == key) return entities[i];
        return null;
    }
