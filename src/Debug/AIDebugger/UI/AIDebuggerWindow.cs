...
internal sealed class AIDebuggerWindow
{
    ...
    private DebugEntityKey selected, compareKey;
    ...
    internal void Draw(RainWorldGame game, double overheadMs)
    {
        ...
        if (AIDebugWorldOverlay.TryPick(game, entities, ImGui.GetIO().WantCaptureMouse, out AbstractCreature picked))
            Select(picked);
    ...
    private AIDebugSnapshot ActiveSnapshot => freezeView && frozenSnapshot != null ? frozenSnapshot : snapshot;
    ...
    private void DrawCompact(double overheadMs)
    {
        ...
        if (active == null)
        {
            ...
            ImGui.TextDisabled(L("Alt + Left Click: pick creature in world", "Alt + 左键：从世界画面选择生物"));
            ImGui.End();
            return;
        }
        ...
        if (freezeView) { ImGui.SameLine(); ImGui.TextColored(new Num.Vector4(0.95f, 0.70f, 0.28f, 1f), L("FROZEN", "已冻结视图")); }
        ...
        {
            AIDebugTraceEvent latest = events[^1];
            ImGui.Separator();
    ...
    private void DrawFull(RainWorldGame game, double overheadMs)
    {
        ...
        ImGui.SetNextWindowSize(new Num.Vector2(Mathf.Max(760f, Screen.width - 16f), Mathf.Max(480f, Screen.height - 16f)), ImGuiCond.Always);
    ...
    private void DrawTopControls(double overheadMs)
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button(freezeView ? L("Unfreeze", "解除冻结") : L("Freeze View", "冻结视图")))
            ToggleFreeze();
        ImGui.SameLine();
    ...
    private void DrawEntityBrowser(RainWorldGame game)
    {
        ...
        ImGui.TextDisabled(L("Click = select · Shift+Click = compare", "点击=选择 · Shift+点击=对比对象"));
        ImGui.Separator();
    ...
    private void DrawWorkspace(RainWorldGame game)
    {
        AIDebugSnapshot active = ActiveSnapshot;
        if (active == null)
        {
        ...
        if (ImGui.BeginTabBar("##AIDebugTabs", ImGuiTabBarFlags.FittingPolicyScroll))
        {
    ...
    private void DrawDecisionStack(AIDebugSnapshot active)
    {
        ...
        if (events.Count == 0 && hasSelection) AIDebugTrace.CopyEvents(selected, events);
        if (events.Count > 0)
        {
            ImGui.Separator();
            ImGui.TextDisabled(L("Why / Why not (latest transitions)", "Why / Why Not（最近决策变化）"));
            ...
            for (int i = first; i < events.Count; i++)
            {
                ...
                {
                    ImGui.Indent(14f); ImGui.TextDisabled(e.Reason); ImGui.Unindent(14f);
                }
            }
        }
    }
    ...
    private void DrawInspector(AIDebugSnapshot active)
    {
        ...
        ImGui.SameLine();
        if (ImGui.Button(L("Set Compare B", "设为对比 B")) && selectedCreature != null) SetCompare(selectedCreature);
        ImGui.SameLine();
        ImGui.Text(active.DisplayName);
        ImGui.Separator();
    ...
    private void DrawTimeline()
    {
        ...
        if (ImGui.Button(freezeView ? L("Resume live view", "恢复实时视图") : L("Freeze at latest", "冻结到最新帧")))
            ToggleFreeze();
        ImGui.SameLine();
        ...
        ImGui.SameLine();
        ImGui.TextDisabled($"{frames.Count} samples · ~{frames.Count * 0.1f:0.0}s window");
        ...
        if (ImGui.BeginTable("##timelineFrame", 2, ImGuiTableFlags.BordersInnerV | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingStretchProp))
        {
            ...
            Row("Utility 0 / 1 / 2", $"{f.Utility0:0.000} / {f.Utility1:0.000} / {f.Utility2:0.000}");
    ...
    private void DrawTimelineStrip()
    {
        ...
        for (int i = 0; i < frames.Count; i++)
        {
            ...
            uint color = TimelineColor(frames[i]);
            draw.AddRectFilled(new Num.Vector2(x0, start.Y + 7f), new Num.Vector2(x1, start.Y + height - 7f), color);
        }
        ...
        if (ImGui.IsItemClicked(ImGuiMouseButton.Left))
        {
            ...
            timelineCursor = Mathf.Clamp(Mathf.FloorToInt(localX / Mathf.Max(0.001f, step)), 0, frames.Count - 1);
    ...
    private void DrawEvents()
    {
        ...
        for (int i = 0; i < events.Count; i++)
        {
            ...
            Num.Vector4 color = EventColor(e.Category);
            ImGui.TextColored(color, $"[{e.Frame}] {e.Category}");
            ImGui.SameLine();
            ImGui.Text(e.Name);
        ...
        if (shown == 0) ImGui.TextDisabled(L("No event matches the filter.", "没有匹配筛选条件的事件。"));
        if (autoScrollEvents && ImGui.GetScrollY() >= ImGui.GetScrollMaxY() - 24f) ImGui.SetScrollHereY(1f);
        ImGui.EndChild();
    }
    ...
    private void DrawUtility()
    {
        ...
        ImGui.TextDisabled(selectedCreature.realizedCreature is DesertBatfly
            ? L("DesertBatfly role scores are shown as custom utilities.", "沙漠蝠蝇的角色评分作为自定义效用显示。")
            : L("Values come from the live vanilla UtilityComparer trackers.", "数据直接来自原版 UtilityComparer 的实时 tracker。"));
            ...
            ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.Resizable | ImGuiTableFlags.ScrollY))
        {
            ...
            for (int i = 0; i < utilities.Count; i++)
            {
                ...
                if (u.Winner) ImGui.TextColored(new Num.Vector4(0.42f, 0.90f, 0.55f, 1f), "YES");
        ...
        if (selectedCreature.realizedCreature is DesertBatfly bat)
        {
            ...
            if (bat.room != null && DesertSwarmRoom.TryGet(bat.room, out DesertSwarmRoom colony)) flock = colony.Flock;
    ...
    private void DrawPerception()
    {
            ...
            ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.Resizable | ImGuiTableFlags.ScrollY))
        {
            ...
            for (int i = 0; i < perception.Count; i++)
            {
                ...
                ImGui.TableSetColumnIndex(7); ImGui.Text($"{p.Relationship} {p.RelationshipIntensity:0.00}");
            }
            ImGui.EndTable();
        }
    }
    ...
    private void DrawPath()
    {
        ...
        if (ImGui.BeginTable("##pathChain", 3, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingStretchProp))
        {
            ImGui.TableSetupColumn(L("Layer", "层级"));
            ImGui.TableSetupColumn(L("Value", "值"));
            ImGui.TableSetupColumn(L("Meaning", "含义"));
            ImGui.TableHeadersRow();
            PathRow("INTENT", path.Destination.ToString(), L("Abstract AI destination / desired world coordinate", "抽象 AI 的世界目标坐标"));
            PathRow("PLANNER", path.HasPathfinder ? path.Pathfinder : "—",
                path.HasPathfinder ? $"reachable={path.DestinationReachable}, returnable={path.CanReturnFromDestination}, stranded={path.Stranded}" : L("No PathFinder module", "没有 PathFinder 模块"));
            ...
            if (selectedCreature.realizedCreature is DesertBatfly bat)
                ...
                PathRow("MOTOR", AIDebugFormat.Value(selectedCreature.realizedCreature?.mainBodyChunk?.pos),
                    selectedCreature.realizedCreature == null ? L("Abstract / unrealized", "抽象 / 未实体化") : L("Creature physics / species motor", "生物物理 / 物种运动执行"));
            ImGui.EndTable();
        }

        ImGui.Spacing();
        ImGui.TextDisabled(L("Intent → Planner → Motor is displayed separately so a correct destination can be distinguished from a bad path or bad locomotion command.",
            "将 Intent → Planner → Motor 分开显示，用于区分“目标正确但寻路错”与“寻路正确但运动执行错”。"));
    }

    private void DrawCompare(RainWorldGame game)
    {
        if (!hasCompare || compareCreature == null || compareSnapshot == null)
        {
            ImGui.TextDisabled(L("Shift+Click another creature in the Entity Browser to set comparison B.",
        ...
        {
            hasCompare = false; compareCreature = null; compareSnapshot = null;
            return;
        }
        ImGui.SameLine();
    ...
    private void RefreshEntities(RainWorldGame game)
    {
        ...
        if (!hasSelection && entities.Count > 0)
        {
            ...
            for (int i = 0; i < entities.Count; i++)
            {
                ...
                if (creature.realizedCreature is DesertBatfly) { candidate = creature; break; }
    ...
    private void Select(AbstractCreature creature)
    {
        ...
        AIDebugTrace.Record(selected, AIDebugEventCategory.State, "Selected", selected.ToString(), "world/browser selection");
    }
    ...
    private void ToggleFreeze()
    {
        if (!freezeView)
        {
            ...
            freezeView = true;
            if (hasSelection && AIDebugTrace.CopyFrames(selected, frames) > 0) timelineCursor = frames.Count - 1;
        }
        else
        {
            freezeView = false;
    ...
    private void ResolveSelection(RainWorldGame game)
    {
        ...
        if (selectedCreature == null || selectedCreature.slatedForDeletion || DebugEntityKey.From(selectedCreature) != selected)
            selectedCreature = FindEntity(selected) ?? AIDebugRegistry.Resolve(game, selected);
        if (selectedCreature == null) { snapshot = null; return; }
    ...
    private void ResolveCompare(RainWorldGame game)
    {
        ...
        if (compareCreature == null || compareCreature.slatedForDeletion || DebugEntityKey.From(compareCreature) != compareKey)
            compareCreature = FindEntity(compareKey) ?? AIDebugRegistry.Resolve(game, compareKey);
        if (compareCreature == null) { compareSnapshot = null; return; }
    ...
    private void SampleTrackedEntities()
    {
        ...
        {
            AbstractCreature creature = FindEntity(key);
            if (creature != null && !ReferenceEquals(creature, selectedCreature)) SampleTrace(creature);
        }
    }
    ...
    private static void SampleTrace(AbstractCreature creature)
    {
        ...
        string utility = ai?.utilityComparer?.HighestUtilityModule()?.GetType().Name ?? "—";
        ...
        float highest = ai?.utilityComparer?.HighestUtility() ?? 0f;
        float threat = ai?.threatTracker?.Utility() ?? 0f;
        AIDebugTrace.Sample(creature, new AIDebugTraceFrame(creature.Room?.name, pos, vel, pos,
            controller, utility, "—", "—", controller, highest, threat, 0f, threat));
    }
    ...
    private static void Flatten(AIDebugSnapshot source, Dictionary<string, string> output)
    {
        ...
        for (int s = 0; s < source.Sections.Count; s++)
            for (int i = 0; i < source.Sections[s].Values.Count; i++)
            {
                ...
                string name = !string.IsNullOrEmpty(value.RawName) ? value.RawName : AIDebugLocalization.T(value.LabelKey);
    ...
    private static bool EventMatches(AIDebugTraceEvent e, string filter)
    {
        ...
        return Contains(e.Category.ToString(), filter) || Contains(e.Name, filter) || Contains(e.Detail, filter) || Contains(e.Reason, filter);
    }
    ...
    private static string DesertRoleExplanation(DesertBatfly bat, DesertBatflyFlockSnapshot flock)
    {
        DesertBatflySocialRoles roles = bat.DesertAI.Roles;
        if (roles.Suppression != SocialRoleSuppression.None) return L("Blocked by ", "被以下状态阻止：") + roles.Suppression;
        if (roles.Cooldown > 0) return L("Role entry cooldown: ", "角色进入冷却：") + roles.Cooldown;
        if (bat.DesertAI.FormalAttack) return L("Formal attack currently owns behavior.", "正式攻击状态机当前拥有行为控制权。");
        DesertBatflyRoleScores s = roles.Scores;
        ExpressedSocialRole best = s.Bully > s.Sentinel ? ExpressedSocialRole.Bully : ExpressedSocialRole.Sentinel;
        ...
        if (bestScore < threshold) return $"Why not {best}: score {bestScore:0.000} < threshold {threshold:0.000}";
        if (bestScore - second < 0.12f) return $"Why not {best}: lead {bestScore - second:0.000} < 0.120";
