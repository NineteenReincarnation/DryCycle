...
{
    private static void RunEnvironmentRain()
    {
        ...
        Check(Resolve("HeavyRain", Sample("HEAVYRAIN", 0.70f, 1f, 0.80f, 0.85f, int.MaxValue)) == "Sheltering",
            "Environment HeavyRain reaches covered-space Sheltering at strong DryCycle intensity");
        Check(Resolve("HeavyRain", Sample("HEAVYRAIN", 0.34f, 1f, 0.45f, 0.55f, int.MaxValue)) == "Preparation",
            "Environment HeavyRain contracts through Preparation before sheltering");
        Check(Resolve("HeavyRain", Sample("HEAVYRAIN", 0.70f, 1f, 1f, 1f, 0)) != "Acute",
            "Environment HeavyRain alone has no Acute phase; DeathRain owns lethal rain survival");

        MethodInfo heavyRainBurden = profile.GetMethod("HeavyRainBurden", Flags);
        Check(heavyRainBurden != null,
            "Environment HeavyRain exposes an explicit realized RainExposure/RoofShielding burden model");
        ...
        Check(exposed > roofed + 0.35f,
            "Environment HeavyRain roof shielding strongly reduces realized rain burden");
        Check(injured > exposed,
            "Environment HeavyRain injured/shocked individuals contract earlier under equal exposure");
        ...
        Check(Math.Abs((float)visibility.Invoke(null, new object[] { heavyRainWeather, 1f }) - 1f) < 0.0001f &&
            ...
            "Environment HeavyRain does not steal Fog visibility/navigation semantics");
        ...
        Check(MethodCallOffset(applyWeather, profile, "HeavyRainBurden") >= 0,
            "Environment HeavyRain behavior consumes the local burden model instead of intensity-only sheltering");
        ...
        Check(deathPreparation > deathShelter && deathShelter > 0,
            "Environment DeathRain has bounded pre-onset preparation before hard local sheltering");
        Check(Resolve("DeathRain", Sample("DEATHRAIN", 0f, 0f, 0.35f, 0.45f, deathPreparation - 100)) == "Preparation",
            "Environment DeathRain bounded forecast produces pre-onset local Preparation");
        Check(Resolve("DeathRain", Sample("DEATHRAIN", 0.10f, 0.20f, 0.70f, 0.80f, deathShelter - 100)) == "Sheltering",
            "Environment DeathRain near onset enters local Sheltering while Travel/Colony owns cross-room travel");
        ...
        Check(weatherQuality != null,
            "Environment rain shelter selection remains on the shared bounded multi-anchor scorer");

        Console.WriteLine(
            "Environment rain completion: HeavyRain roof/exposure/injury ecology, non-Acute ceiling, Fog separation, DeathRain forecast escalation and native survival handoff verified.");
    }
}
